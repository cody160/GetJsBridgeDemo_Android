...
public class LinearLayoutSoftKeyboardDetect extends LinearLayout {
    ...
    private static final String TAG = "LinearLayoutSoftKeyboardDetect";

    private int oldHeight = 0;  // Need to save the old height as not to send redundant events
    private int oldWidth = 0; // Need to save old width for orientation change
    private int screenWidth = 0;
    private int screenHeight = 0;
    private GetBaseWebViewActivity app = null;

    public LinearLayoutSoftKeyboardDetect(Context context, int width, int height) {
        super(context);
        screenWidth = width;
        screenHeight = height;
        app = (GetBaseWebViewActivity) context;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        GetJSLog.v(TAG, "We are in our onMeasure method");
        ...
        GetJSLog.v(TAG, "Old Height = %d", oldHeight);
        GetJSLog.v(TAG, "Height = %d", height);
        GetJSLog.v(TAG, "Old Width = %d", oldWidth);
        GetJSLog.v(TAG, "Width = %d", width);
        ...
        if (oldHeight == 0 || oldHeight == height) {
            GetJSLog.d(TAG, "Ignore this event");
        }
        ...
        else if (screenHeight == width)
        {
            ...
            screenWidth = tmp_var;
            GetJSLog.v(TAG, "Orientation Change");
        }
